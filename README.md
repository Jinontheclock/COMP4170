...
**Files:** `index.js`, `package.json`, `dump.sql`

Handled the file structure, dependencies, database, and result stat. Created database and later migrated it to Supabase using `dump.sql` so the site could be deployed publicly on Vercel. Also added logic in `index.js` that queries result_type to show users what percentage of other users got the same result.
